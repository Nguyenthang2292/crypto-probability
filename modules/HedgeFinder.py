...
class HedgeFinder:
    ...
    def list_candidate_symbols(self, exclude_symbols: Optional[set] = None,
        ...
        try:
            # Use authenticated manager for authenticated calls (load_markets requires auth)
        ...
        for market in markets.values():
            ...
            if not market.get('active', True):
                progress.update()
                continue
            symbol = self.exchange_manager.normalize_symbol(market['symbol'])
    ...
    def _score_candidate(self, symbol: str) -> Optional[Dict[str, float]]:
        ...
        weighted_corr, _ = self.correlation_analyzer.calculate_weighted_correlation(symbol, verbose=False)
    ...
    def analyze_new_trade(self, new_symbol: str, total_delta: float, total_beta_delta: float,
        ...
        weighted_corr, weighted_details = self.correlation_analyzer.calculate_weighted_correlation(new_symbol)
        portfolio_return_corr, portfolio_return_details = self.correlation_analyzer.calculate_portfolio_return_correlation(new_symbol)
        ...
        if last_var_value is not None and last_var_confidence is not None:
            ...
            print(color_text(f"  With {conf_pct}% confidence, daily loss is unlikely to exceed {last_var_value:.2f} USDT.", Fore.WHITE))
